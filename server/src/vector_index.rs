...
pub fn record_vectors(db: &sled::Db, doc_id: &str, chunk_starts: &[usize], vector_dim: usize) -> Result<()> {
	let meta = db.open_tree("vec_meta")?;
	let items_key = b"items";
	let dim_key = b"dim";
	// update items count
	let prev = meta.get(items_key)?.map(|v| u64::from_le_bytes(v.as_ref().try_into().unwrap_or([0u8;8]))).unwrap_or(0);
...
pub fn validate_mem_embeddings(db: &sled::Db) -> (u64, u64) {
    ...
    if let Ok(tree) = db.open_tree("mem_embeddings") {
        for kv in tree.iter() {
            if let Ok((_, v)) = kv {
                total += 1;
                if v.len() != EMBED_DIM * 4 { invalid += 1; }
            }
        }
    }
    (total, invalid)
...
fn get_mem_embedding(db: &sled::Db, id: &str) -> Option<Vec<f32>> {
    if let Ok(tree) = db.open_tree("mem_embeddings") {
        if let Ok(Some(v)) = tree.get(id.as_bytes()) {
            if v.len() != EMBED_DIM * 4 { return None; }
...
pub fn build_mem_neighbor_graph(db: &sled::Db, m_neighbors: usize) -> Result<u64> {
    ...
    use rayon::prelude::*;
    let entries: Vec<(Vec<u8>, Vec<u8>)> = (0..n).into_par_iter().map(|i| {
        let a = &vecs[i];
        let mut top: Vec<(f32, usize)> = Vec::with_capacity(m_neighbors+1);
        for j in 0..n {
            if i == j { continue; }
            let score = cosine_similarity(a, &vecs[j]);
            if top.len() < m_neighbors { top.push((score, j)); }
            else {
                top.sort_by(|x,y| x.0.partial_cmp(&y.0).unwrap_or(Ordering::Equal));
                if let Some((min_score, _)) = top.first() { if score > *min_score { top[0] = (score, j); } }
            }
        }
        top.sort_by(|a,b| b.0.partial_cmp(&a.0).unwrap_or(Ordering::Equal));
        let arr: Vec<serde_json::Value> = top.into_iter().map(|(s, idx)| serde_json::json!({ "id": ids[idx], "score": s })).collect();
        (ids[i].as_bytes().to_vec(), serde_json::to_vec(&arr).unwrap_or_default())
    }).collect();
    for (k, v) in entries { neigh.insert(k, v)?; }
...
pub fn ann_search_memories(db: &sled::Db, query: &[f32], top_k: usize) -> Vec<(String, f32)> {
    let neigh = db.open_tree("hnsw_mem_neighbors");
    if neigh.is_err() { return search_memories_by_vector(db, query, top_k); }
    let neigh = neigh.unwrap();
    // choose entry: pick first with highest sim among first 16 entries
    let emb = db.open_tree("mem_embeddings");
    if emb.is_err() { return Vec::new(); }
    let emb = emb.unwrap();
    let mut entry_id: Option<String> = None;
    let mut best_sim = -1.0f32;
    for (idx, kv) in emb.iter().enumerate() {
        if idx >= 16 { break; }
        if let Ok((k, v)) = kv { if v.len() == EMBED_DIM * 4 {
            let id = String::from_utf8_lossy(&k).to_string();
            let vec: &[f32] = bytemuck::cast_slice(&v);
            let s = cosine_similarity(query, vec);
            if s > best_sim { best_sim = s; entry_id = Some(id); }
        }}
    }
    let entry = match entry_id { Some(e) => e, None => return Vec::new() };
    // greedy search
    use std::collections::{HashSet, BinaryHeap};
    #[derive(PartialEq)] struct Scored { score: f32, id: String }
    impl Eq for Scored {}
    impl Ord for Scored { fn cmp(&self, other: &Self) -> Ordering { self.score.partial_cmp(&other.score).unwrap_or(Ordering::Equal) } }
    ...
    while let Some(cur) = frontier.pop() {
        ...
        if let Some(vec) = get_mem_embedding(db, &cur) {
            let s = cosine_similarity(query, &vec);
            best.push(Scored { score: s, id: cur.clone() });
        ...
        if let Ok(Some(nv)) = neigh.get(cur.as_bytes()) {
            if let Ok(arr) = serde_json::from_slice::<Vec<serde_json::Value>>(&nv) {
                for item in arr.into_iter().take(8) { // limit branching
                    if let Some(nid) = item.get("id").and_then(|x| x.as_str()) { frontier.push(nid.to_string()); }
                }
            }
        }
        if visited.len() > 1024 { break; }
    }
    let mut out: Vec<(String, f32)> = best.into_sorted_vec().into_iter().rev().map(|s| (s.id, s.score)).collect();
    out.truncate(top_k);
    out
}
...
pub fn reembed_all_memories(db: &sled::Db, batch_size: usize) -> Result<u64> {
    ...
    for kv in mems.iter() {
        ...
        if let Ok(rec) = serde_json::from_slice::<serde_json::Value>(&v) {
            if let Some(id) = rec.get("id").and_then(|x| x.as_str()) { ids.push(id.to_string()); texts.push(rec.get("content").and_then(|c| c.as_str()).unwrap_or("").to_string()); }
        }
    }
    let emb = db.open_tree("mem_embeddings")?;
