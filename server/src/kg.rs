...
pub fn ensure_episode_node(
    ...
    if nodes.get(key.as_bytes())?.is_none() {
        let mut val =
            serde_json::json!({ "type": "Episode", "id": episode_id, "created_at": created_at });
        if let Some(n) = name {
            val["name"] = serde_json::json!(n);
        }
        if let Some(s) = session_id {
            val["session_id"] = serde_json::json!(s);
        }
        nodes.insert(key.as_bytes(), serde_json::to_vec(&val)?)?;
    }
    Ok(())
...
pub fn entities_for_doc(db: &sled::Db, doc_id: &str) -> Result<Vec<String>> {
    ...
    for kv in links.scan_prefix(prefix.as_bytes()) {
        let (k, _) = kv?;
        let key = String::from_utf8(k.to_vec()).unwrap_or_default();
        if let Some((_, ent)) = key.split_once("::") {
            list.push(ent.to_string());
        }
    }
    Ok(list)
}
            ...
            if let Some(pat) = pattern {
                let matches = key.to_lowercase().contains(&pat.to_lowercase())
                    || node
                        .get("label")
                        .and_then(|l| l.as_str())
                        .map(|s| s.to_lowercase().contains(&pat.to_lowercase()))
                        .unwrap_or(false)
                    || node
                        .get("id")
                        .and_then(|l| l.as_str())
...
pub fn tag_entity(db: &sled::Db, entity: &str, tags: &[String]) -> Result<()> {
    ...
    let existing_tags = node
        .get("tags")
        .and_then(|t| t.as_array())
        .cloned()
        .unwrap_or_default();
    let mut tag_set: std::collections::HashSet<String> = existing_tags
        .into_iter()
    ...
    let tags_vec: Vec<serde_json::Value> =
        tag_set.into_iter().map(|t| serde_json::json!(t)).collect();
...
pub fn delete_entity(db: &sled::Db, entity: &str) -> Result<u64> {
    ...
    let src_prefix = format!("Entity::{}->", entity);
    let to_remove_src: Vec<_> = edges
        .scan_prefix(src_prefix.as_bytes())
